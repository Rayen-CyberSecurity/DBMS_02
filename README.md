...
---<img width="669" height="458" alt="Prerequisites" src="https://github.com/user-attachments/assets/b63cc0cd-80f3-442d-82ec-ba146bb874de" />
...
> *Your answer:* In Chen notation, an N:M relationship is shown using a separate diamond between two entities (for example Author — writes — Book). In Crow’s Foot notation in PlantUML, there is no diamond, and the many-to-many relationship is shown directly with symbols at both ends of the line like }|--|{.
...
> *Your answer:* instead of just @startuml, PlantUML may generate a different output filename instead of schema.svg. That would break the workflow because the GitHub Action expects a file specifically called schema.svg to upload as the release artifact.
...
> *Your answer:*No, in the PlantUML diagram I do not need to explicitly draw the join table for the Author–Book many-to-many relationship. PlantUML abstracts that relationship. The join table would appear later when converting the conceptual ER model into the relational database schema.
...
>
> <img width="992" height="591" alt="SVG" src="https://github.com/user-attachments/assets/5af563ab-4bad-49e2-9dcd-a5a82ce3e4e6" />
...
> *Your answer:* echo $? If it prints: 0 then the last command ran successfully.
...
> *Your answer:* schema.svg is not shown as untracked in git status because it is ignored in .gitignore. Since it is a generated artifact, Git is told not to track it.
...
>
> `[insert screenshot]`
> <img width="828" height="112" alt="Task 4 2" src="https://github.com/user-attachments/assets/a8129a8b-8724-4304-bf44-cafada890eb1" />
...
> *Your answer:* git tag only lists tag names.
git tag -v v1.0.0 shows more information like the annotated tag message, tagger information, and verifies the tag signature. The -v flag checks authenticity of the annotated tag.
...
> *Your answer:* One solution would be using caching in GitHub Actions, or using a custom runner/container where PlantUML is already installed. That would avoid installing it every time.
...
>
> <img width="1847" height="964" alt="Task 5 Screenshot 4" src="https://github.com/user-attachments/assets/0c4efc06-cfcd-412c-a12e-edd29609ba20" />
...
> insert it here.
>
> <img width="1859" height="951" alt="Task 5 Screenshot 5" src="https://github.com/user-attachments/assets/842a856e-d5a1-44c6-a3d9-b9eb6ec49453" />
...
> *Your answer:* The Install PlantUML step took much longer than rendering the SVG. It was about several times slower (around 7–10 times). That suggests optimization should focus on reducing installation time, for example through caching.
...
> *Your answer:* Yes, the released schema.svg and the one rendered locally are identical. This shows the build is reproducible, meaning the same source produces the same result locally and in CI.
...
> *Your answer:* With a .puml file, Git can show exactly what changed in each commit, such as new entities or cardinality changes. If the diagram was stored as a binary .drawio file or PNG, those detailed textual changes would be much harder or impossible to track.
...
> *Your answer:* With schema.puml, Git can show text merge conflicts if two people edit the diagram at the same time. With a binary diagram file, that would be much harder. For a team, this means text diagrams make collaboration and merging much easier.
...
> *Your answer:* An annotated tag always points to one fixed commit, while a branch can move when new commits are added. That makes tags better for releases because they guarantee an exact version. A branch could still be useful for maintaining a long-term release branch with bug fixes.
...
> *Your answer:* The CI pipeline removes the manual step of exporting and committing diagram images by hand. It guarantees the released diagram is always automatically generated from the exact source in the repository, which makes it reliable and consistent.
...
>
> <img width="1868" height="1004" alt="Screenshot 2026-04-29 214002" src="https://github.com/user-attachments/assets/1c8ae077-9529-4372-a008-aefdaba401e6" />
